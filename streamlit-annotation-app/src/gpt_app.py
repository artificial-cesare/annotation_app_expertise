...
# TEST MODE: Set to True to only load N speeches for testing
TEST_MODE = True
...
def load_speeches():
    ...
    try:
        ...
        # In test mode, only return first N speeches actually used in this run
        if TEST_MODE:
...
def get_annotations_for_rater(rater_id: str):
    """
    Get all annotations by this rater as a dict indexed by item_id (speech_id).
    
    IMPORTANT: This is called on every rerun so that any change on disk
    (refresh / navigation / submit) is always reflected in the UI.
    """
    if not ANNOTATIONS_FILE.exists():
        return {}
    
    df = pd.read_csv(ANNOTATIONS_FILE)
    # If file exists but has no data or missing cols, fail safe
    if df.empty or ("rater_id" not in df.columns) or ("item_id" not in df.columns):
        return {}
...
def update_annotation(item_id, rater_id, score, justification, context, statement):
    """
    Update existing annotation (speech_id + rater_id) or create new one.
    
    This function OVERWRITES previous information for that (item_id, rater_id)
    combination (score, justification, context, statement), satisfying requirement (1).
    """
    annotation_row = {
        "item_id": item_id,
        "rater_id": rater_id,
    ...
    # If file does not exist, create it with this single row
    if not ANNOTATIONS_FILE.exists():
        df = pd.DataFrame([annotation_row])
    ...
    # Ensure columns exist; if not, re-create with proper schema
    expected_cols = ["item_id", "rater_id", "score", "justification", "context", "statement"]
    for col in expected_cols:
        if col not in df.columns:
            df[col] = ""  # add missing columns with empty values
    
    # Check if annotation exists for this (item_id, rater_id)
    mask = (df['item_id'] == item_id) & (df['rater_id'] == rater_id)
    
    if mask.any():
        # OVERWRITE existing row for this combination
        df.loc[mask, ["score", "justification", "context", "statement"]] = [
            score,
            justification,
            context,
            statement
        ...
        # Append as new row
        new_row = pd.DataFrame([annotation_row])
...
def main():
    ...
    with st.sidebar:
        ...
        rater_id = st.text_input("Enter your Rater ID:", key="rater_id")
    ...
    # ALWAYS reload annotations for this rater from disk on every rerun
    annotations = get_annotations_for_rater(rater_id)
    
    # Speech IDs in the current session (handles TEST_MODE varying N)
    speech_ids_to_annotate = [s['speech_id'] for s in speeches]
    
    # Count how many of THESE speeches have been annotated for this rater
    ...
    # If annotator is done with all currently loaded speeches,
    # show Finish button (works in both full and TEST modes).
    if completed >= total_speeches and total_speeches > 0:
        st.success("🎉 You have completed all annotations for the loaded speeches!")
        st.balloons()
        ...
        if st.button("✅ Finish and Close", type="primary", use_container_width=True):
            ...
            # st.stop() effectively "closes" the app from Streamlit's side.
            st.stop()
        
        # Ensure nothing else is rendered once finished view is shown
        st.stop()
    
    # Get current speech index
    ...
    # Retrieve existing annotation for this speech_id + rater_id combination
    existing = annotations.get(current_speech['speech_id'], {})
    
    # Use existing values if present, otherwise use neutral defaults
    default_score = existing.get('score', 3)
    default_justification = existing.get('justification', '')
    
    # Unique form key per speech_id ensures the widgets refresh when speech changes
    form_key = f"form_{current_speech['speech_id']}"
    ...
    with st.form(key=form_key):
        ...
        if submitted:
            if not justification.strip():
                ...
                # Build a flat context string
                context_list = current_speech.get('context', [])
                context_str = " | ".join(context_list) if context_list else ""
                
                # OVERWRITE or add new annotation for this (speech_id, rater_id)
                update_annotation(
                ...
                # After saving, auto-advance if not at the last speech
                ...
                # - Update progress
                # - Potentially show the Finish button if this was the last missing one
                st.rerun()
